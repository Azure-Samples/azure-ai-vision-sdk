...
/// client.
/// </summary>
internal sealed class TokenHandler : HandlerBase
{
    public const string Route = "session/token";

    public TokenHandler(AttestationContext ctx) : base(ctx) { }

    public async Task<HandlerOutcome> HandleAsync(SessionTokenRequest req)
    {
        ...
        string? msgChallenge, msgClientId, msgSystem;
        try
        {
            var parsed = JsonNode.Parse(decryptedPayload) as JsonObject;
            msgChallenge = parsed is null ? null : JsonHelpers.GetString(parsed, "challengeHash");
            msgClientId = parsed is null ? null : JsonHelpers.GetString(parsed, "clientId");
            msgSystem = parsed is null ? null : JsonHelpers.GetString(parsed, "system");
        }
        catch (JsonException)
        {
        ...
        if (string.IsNullOrEmpty(msgChallenge) || string.IsNullOrEmpty(msgClientId) || string.IsNullOrEmpty(msgSystem) ||
            msgChallenge != JsonHelpers.GetString(data, "challengeHash") ||
            msgClientId != JsonHelpers.GetString(data, "clientId") ||
            msgSystem != JsonHelpers.GetString(data, "system"))
        {
            return Fail(Route, 401, "PAYLOAD_MISMATCH", "Payload mismatch", new Dictionary<string, object?>
            {
                ["sid"] = sessionId,
                ["hasChallenge"] = !string.IsNullOrEmpty(msgChallenge),
                ["hasClientId"] = !string.IsNullOrEmpty(msgClientId),
                ["hasSystem"] = !string.IsNullOrEmpty(msgSystem),
            });
        }

        data["authCompleted"] = true;
        var responsePayload = JsonSerializer.Serialize(new { token = sessionData.Token, timestamp = IsoTime.Now() });
        ...
        return Ok(new SessionTokenSuccess { EncryptedData = encryptedResponse });
    }
}
