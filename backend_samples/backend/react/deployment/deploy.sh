...
DEPLOY_NAME="liveness-react-$ENV-$(date +%Y%m%d%H%M%S)"
...
WEBAPP_HOST=$(echo "$OUTPUTS" | python3 -c "import sys,json;print(json.load(sys.stdin)['webAppHost']['value'])")
...
echo "==> Packaging Next.js standalone bundle..."
STANDALONE="$REPO_ROOT/.next/standalone"
cp -r "$REPO_ROOT/.next/static" "$STANDALONE/.next/static"
[[ -d "$REPO_ROOT/public" ]] && cp -r "$REPO_ROOT/public" "$STANDALONE/public"

ZIP="$(mktemp -d)/liveness-react-app.zip"
( cd "$STANDALONE" && zip -r -q "$ZIP" . )
...
echo ""
echo "Next: bind the mobile samples to this host:"
echo "  ../scripts/set-domain.sh $WEBAPP_HOST"
