...
var redisName = 'liveness-react-${env}-${locationabbr}'
...
// Single database per cluster — the convention for Managed Redis is to name it 'default'.
// EnterpriseCluster policy exposes a single endpoint so the standard @redis/client
// (not the cluster client) keeps working unchanged.
