...
1. The solution uses azure maven repo artifact to add the binary enabling the liveness feature.  You will need to set up azure `maven` repo with any `username` and valid "access token" as "password.  It will be mentioned below in [Get Access Token to SDK Artifact](#step-34-get-access-token-to-sdk-artifact) section for getting the password, along with the [Add Build Dependencies](#step-35-add-build-dependency) to set the repo in the solution files.
2. The app requires camera permission.  You will need to set it up in the app configuration and code.  It will be mentioned below in [Add Camera Permission](#step-32-add-camera-permissions) and [Add Kotlin code to Request Camera Permission](#step-33-add-kotlin-code-to-request-camera-permission) sections for demonstration.
...
4. The compose method takes a set of parameters launching it, the parameters defines the liveness session and callback behaviour.  It will be mentioned below in [Add code to interpret the result](#step-36-add-code-to-interpret-the-result) section to demostrate how to use it.
...
* You need to add credentials in gradle.properties to set up variable `mavenUser` and `mavenPassword` used above. These are obtained through azure command from above [Get Access Token to SDK Artifact](#step-34-get-access-token-to-sdk-artifact) section.
...
* If you use ProGuard or DexGuard, please refer to [ProGuard Exceptions](#step-391-proguard-exceptions) and [DexGuard Exceptions](#step-392-dexguard-exceptions) for details.
...
* digest The validation string to be used to verify the communication for this call is secure.  For more information check section below [Add validation for the integrity of the service result](#step-38-add-validation-for-the-integrity-of-the-service-result)
...
1. "digest" property in LivenessDetectionSuccess shown in [Step 5 Add code to interpret the result](#step-36-add-code-to-interpret-the-result)
