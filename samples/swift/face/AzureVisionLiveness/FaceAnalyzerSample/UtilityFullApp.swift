...
extension SessionData {
    func sessionResultMessage(livenessDetectionResult: LivenessDetectionResult?) -> String {
        ...
        case .success:
            let defaultString = "Check Complete"
            // AppLink/QuickLink flow doesn't have endpoint+key for the
            // server-side result fetch and the result screen only shows
            // "Check Complete" anyway, so skip the network round trip.
            if MainActor.assumeIsolated({ AppLinkState.shared.isFromAppLink }) {
